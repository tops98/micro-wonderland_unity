...
public class AutoLabel : MonoBehaviour
{
    ...
    [SerializeField]bool _useOcclusionInfo = false;
    [SerializeField]bool _useTimestamp = false;
    ...
    List<Anotation> GetBoundingBoxes()
    {
        ...
        foreach (var obj in _trackedObjects)
        {
            ...
            if(anotation.min.x <0 || anotation.min.x > res.x){
                continue;
            }

            if(anotation.max.x <0 || anotation.max.x > res.x){
                continue;
            }
            if(anotation.min.y <0 || anotation.min.y > res.y){
                continue;
            }

            if(anotation.max.y <0 || anotation.max.y > res.y){
                continue;
            }
            ...
            anotation.occluded = IsOccluded(coll.transform.position);
            anotation.timestamp = DateTimeOffset.Now.ToUnixTimeMilliseconds();
    ...
    private bool IsOccluded(Vector3 pos)
    {
        ...
        Vector3 origin = new Vector3(pos.x, height, pos.z);
        RaycastHit hit;
        Ray ray = new Ray(origin, Vector3.down);
        
        
        if(Physics.Raycast( ray, out hit, height) && (hit.transform.tag == "Occluder"))
        {
            Debug.DrawRay( origin, Vector3.down * height, Color.red);
            return true;
        }

        Debug.DrawRay( origin, Vector3.down * height, Color.green);
        return false;
    }
    ...
    string CreateannotationsString()
    {
        ...
        foreach (var anotation in _annotations)
        {
            ...
            string occludedInfo = (_useOcclusionInfo)?$@",""occluded"":{anotation.occluded.ToString().ToLower()}":"";
            Debug.Log(anotation.timestamp);
            string timestampInfo = (_useTimestamp)?$@",""timestamp"":{anotation.timestamp}":"";
            Debug.Log(anotation.occluded);
            sb.Append($@"
                {{
                    ""id"": {anotation.id},
                    ...
                    ""area"": {width*height},
                    ""iscrowd"": 0 {occludedInfo}{timestampInfo}
                }},");
        }
        sb.Remove(sb.Length-1,1);
        return sb.ToString();
